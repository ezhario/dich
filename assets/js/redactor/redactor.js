	...
	Construct.prototype = {
		...
		*/	
		...
		getEditorDoc: function(html)
		{
			...
			frameHtml += '<html><head>' + css + '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" /></head><body class="content">';
			frameHtml += html;
			frameHtml += '</body></html>';
			return frameHtml;
		},	
		...
		*/		
		...
		{	
			if (this.opts.fullscreen === false)
			{
				...
				this.box.css({ position: 'absolute', top: 0, left: 0, zIndex: 1000 }).after('<span id="fullscreen_' + this.frameID +  '"></span>');
		...
		*/
		...
		docObserve: function()
		{
			$(this.doc).find('body').find('img').unbind('click');
			$(this.doc).find('body').find('table').unbind('click');
			
			$(this.doc).find('body').find('img').bind('click', function(e) { this.imageEdit(e); }.bind(this) );
			$(this.doc).find('body').find('table').bind('click', function(e) { this.showTable(true, e); }.bind(this) );
		},			
		...
		*/
		...
		{
			RedactorActive = this;
			this.modalInit(RLANG.video, this.opts.path + 'plugins/video.html', 600, 'auto');
		},	
		...
		*/		
		showCharmap: function()
		{
			...
			this.modalInit(RLANG.charmap, this.opts.path + 'plugins/char.html', 600, 'auto', handler);
		...
		*/	
		imageEdit: function(e)
		{
			...
			RedactorActive = this;		
			this.modalInit(RLANG.image, this.opts.path + 'plugins/image_edit.html', 350, 'auto', handler);
		},
		imageSave: function(el)
		{
			...
			if ($('#redactor_form_image_align') != 0)
			{
				...
				if (float == 'left') $(el).css({ float: 'left', clear: 'left' }).removeClass("right").addClass("left");
				else if (float == 'right')  $(el).css({ float: 'right', clear: 'right' }).removeClass("left").addClass("right");
			}
			else $(el).css({ float: 'none' }).removeClass("right").removeClass("left");
		...
		showImage: function()
		{
			...
			RedactorActive = this;
			this.modalInit(RLANG.image, this.opts.path + 'plugins/image.html', 450, 'auto', handler);
		},
		imageUploadCallback: function(psData)
		{
			poData = $.parseJSON( psData );
			...
			if (poData.length > 0)
			{
				...
				if ($('#redactor_form_image_align') != 0)
				{
					...
					if (float == 'left')  { style = 'style="float: left; clear: left;"'; aclassname='class="left"'; }
					else if (float == 'right') {style = 'style="float: right; clear: right;"'; aclassname='class="right"'; };
				
					var html = '<img alt="' + alt + '" src="' + data + '" ' + style + ' ' + aclassname + ' />';
				}
				else
				{
		...
		*/				
		showLink: function()
		{
			...
			this.modalInit(RLANG.link, this.opts.path + 'plugins/link.html', 400, 'auto', handler);
			...
			$.each(RTOOLBAR, 
	   			function (i, s)
	   			{
	   				...
	   				else
	   				{
						...
						{
							$(li).addClass("imp_redactor_drop_down_li");
		...
		*/
		...
		{
			$('#imp_redactor_toolbar_' + this.frameID + ' li.imp_redactor_drop_down_li').removeClass('act');
		...
		*/
		...
		modalInit: function(title, url, width, height, handler, scroll)
		{
			$.ajax({
				...
				{		
					...
					/*$( '<div style="position: relative; width:100%; min-height: 400px;"></div>' )
						.html(data)*/
						var loInstance = $('<div class="some-redactor-dialog"></div>');
		...
		modalClose: function()
		{
			$(".some-redactor-dialog").dialog("close");
		},
